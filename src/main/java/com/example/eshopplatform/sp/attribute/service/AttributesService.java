package com.example.eshopplatform.sp.attribute.service;

import com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.example.eshopplatform.sp.attribute.mapper.AttributesMapper;
import com.example.eshopplatform.sp.attribute.entity.Attributes;
...
public class AttributesService {
    ...
    public PageResult<AttributesVO> page(int page, int size, Long categoryId, Integer searchable, Integer isSkuSpec) {
        Page<Attributes> p = new Page<>(normalizePage(page), normalizeSize(size));
        attributesMapper.selectPage(p, new LambdaQueryWrapper<Attributes>()
                .eq(categoryId != null && categoryId > 0, Attributes::getCategoryId, categoryId)
                .eq(searchable != null && searchable >= 0, Attributes::getSearchable, searchable)
                .eq(isSkuSpec != null && isSkuSpec >= 0, Attributes::getIsSkuSpec, isSkuSpec)
                .orderByDesc(Attributes::getId));
        return PageResult.of(p.getTotal(), p.getRecords().stream().map(this::toVO).toList());
    }

    /**
     * 可搜索属性列表（对齐 gf ListSearchable）：searchable=1 且启用，
     * 可选按推荐类目过滤，按 sort_order,id 升序，供前台筛选条件来源使用。
     */
    public List<AttributesVO> listSearchable(Long categoryId) {
        return selectForPicker(categoryId, true);
    }

    /**
     * SKU 规格属性列表（对齐 gf ListSkuSpec）：is_sku_spec=1 且启用，
     * 可选按推荐类目过滤，按 sort_order,id 升序，供 SPU 详情页规格选择使用。
     */
    public List<AttributesVO> listSkuSpec(Long categoryId) {
        return selectForPicker(categoryId, false);
    }

    /** 下拉/选择类列表公共查询：按 searchable 或 is_sku_spec 标记 + 启用态过滤 */
    private List<AttributesVO> selectForPicker(Long categoryId, boolean searchable) {
        LambdaQueryWrapper<Attributes> wrapper = new LambdaQueryWrapper<Attributes>()
                .eq(searchable ? Attributes::getSearchable : Attributes::getIsSkuSpec, (byte) 1)
                .eq(Attributes::getStatus, (byte) 1)
                .eq(categoryId != null && categoryId > 0, Attributes::getCategoryId, categoryId)
                .orderByAsc(Attributes::getSortOrder)
                .orderByAsc(Attributes::getId);
        return attributesMapper.selectList(wrapper).stream().map(this::toVO).toList();
    }
    ...
    /** 按主键查询 */
    public AttributesVO getById(Long id) {
        return toVO(require(id));
    }

    /** 新增 */
    public AttributesVO create(AttributesCreateReq req) {
        Attributes entity = new Attributes();
        apply(entity, req);
        attributesMapper.insert(entity);
        return toVO(entity);
    }
     ...
     */
    public AttributesVO update(Long id, AttributesUpdateReq req) {
        Attributes entity = require(id);
        apply(entity, req);
        attributesMapper.updateById(entity);
        return toVO(entity);
    }
    ...
    public void delete(Long id) {
        require(id);
        attributesMapper.deleteById(id);
    }

    /** 按主键查询，不存在抛 404 */
    private Attributes require(Long id) {
        Attributes entity = attributesMapper.selectById(id);
        if (entity == null) {
            throw BizException.notFound("属性字典表不存在");
        }
        return entity;
    }

    /** entity -> VO（生成器自动映射：deleted_at 不对外暴露；时间列转 Long epoch 毫秒） */
    private AttributesVO toVO(Attributes entity) {
        AttributesVO vo = new AttributesVO();
        vo.setId(entity.getId());
        vo.setName(entity.getName());
        vo.setCategoryId(entity.getCategoryId());
        vo.setValueType(entity.getValueType());
        vo.setFilterable(entity.getFilterable());
        vo.setUnit(entity.getUnit());
        vo.setRequired(entity.getRequired());
        vo.setSearchable(entity.getSearchable());
        vo.setIsSkuSpec(entity.getIsSkuSpec());
