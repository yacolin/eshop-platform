...
-- ============================================================
-- sp_p3.sql — 库存相关表（依赖 P2: sp_skus 与 sp_p4: sp_warehouses）
-- ============================================================
...
CREATE TABLE `sp_inventory_logs` (
  ...
  `reference_id_uq` varchar(64) GENERATED ALWAYS AS (NULLIF(`reference_id`, '')) STORED COMMENT '幂等键辅助列（空串视为NULL，无单据操作不占用唯一键）',
  `operator` varchar(50) DEFAULT '' COMMENT '操作人（系统操作填 system）',
  `note` varchar(500) DEFAULT '' COMMENT '备注',
  `created_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ref_type` (`reference_id_uq`, `change_type`) COMMENT '幂等约束：同单据同类型操作不重复（空串无单据操作不冲突）',
